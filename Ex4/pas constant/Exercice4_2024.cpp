...
main(int argc, char* argv[])
{
    ...
    fichier_T    = configFile.get<string>("output") + "_T.out"; // Output file for temperature
    fichier_heat = configFile.get<string>("output") + "_heat.out"; // Output file for heat flux
    ...
    // "uniforme" de type bool qu'il faut récupérer de notre configuratio.in
    ...
    for (int k = 0; k < N; ++k) { 
        ...
        rhs[k]     += rk_demi * h[k]*Source(rk_demi)/2; 
        rhs[k + 1] += rk_demi * h[k]*Source(rk_demi)/2;
