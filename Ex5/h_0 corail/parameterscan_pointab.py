...
err_WKB_1 = np.abs(A - A_WKB_1)

ax.plot(x,err_WKB_1, label=f"Erreur amplitude WKB éq.1", color="black")
...
plt.tight_layout()
u.set_legend_properties(ax, fontsize=18,ncol=1)
u.savefig(fig, "Erreur_Amplitude", ext = ext)
...
plt.tight_layout()
u.set_legend_properties(ax, fontsize=18,ncol=1)
u.savefig(fig, "Erreur_Vitesse", ext = ext)
