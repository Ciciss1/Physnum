...
# #redef nsteps ? 
# nsteps = np.array([10000,15000,20000,40000,60000,80000,100000,125000,150000,175000,200000]) # TODO change as you need
# nsimul = len(nsteps)  # Number of simulations to perform
# dt = 1/nsteps # define it as you need
...
plt.tight_layout()
u.savefig(fig, "ExcitationChaos_convergence_dtpetits",ext)
...
# u.zoom_in_plot(ax,x,y,xlim,ylim,x_zoom,y_zoom,width_zoom,height_zoom,"navy")
...
# #-----------plot xy for different dt------------
# ax,fig = u.create_figure_and_apply_format(figsize=(10, 8),xlabel=r"$x$ [m]", ylabel=r'$y$ [m]', grid_bool=False)

# #i want to begin at t=tfin/2

# for i in [0,10,-1] : 
#     t = np.linspace(0,tfin,len(thetasimul_list[i]))
#     l = alpha*t + L + d*np.sin(omega*t)
#     y = -l*np.cos(thetasimul_list[i])
#     x = l*np.sin(thetasimul_list[i])
#     ax.plot(x,y,linestyle = "-", label = r"$N_{steps}$" + f" = {nsteps[i]}")

# plt.tight_layout()
# ax.grid()
# u.set_legend_properties(ax)
# u.savefig(fig, "ExcitationChaos_xy_diffNsteps",ext)



#-----------plot thetadot(t) for different dt------------
ax,fig = u.create_figure_and_apply_format(figsize=(10, 8),ylabel=r"$\dot{\theta}(t)$ [rad]", xlabel=r'$t$ [s]', grid_bool=False)

for i in [1,10,-1] : 
    t = np.linspace(0,tfin,len(thetasimul_list[i]))
    
    ax.plot(t,thetadotsimul_list[i],linestyle = "-", label = r"$N_{steps}$" + f" = {nsteps[i]}")

plt.tight_layout()
ax.grid()
u.set_legend_properties(ax)
u.savefig(fig, "ExcitationChaos_thetadot(t)_diffNsteps",ext)


# #-----------plot theta(t) for different dt------------
# ax,fig = u.create_figure_and_apply_format(figsize=(10, 8),ylabel=r"$\theta(t)$ [rad]", xlabel=r'$t$ [s]', grid_bool=False)

# for i in [1,10,-1] : 
#     t = np.linspace(0,tfin,len(thetasimul_list[i]))
    
#     ax.plot(t,thetasimul_list[i],linestyle = "-", label = r"$N_{steps}$" + f" = {nsteps[i]}")

# plt.tight_layout()
# ax.grid()
# u.set_legend_properties(ax)
# u.savefig(fig, "ExcitationChaos_theta(t)_diffNsteps",ext)
...
# #-----------plot portrait de phase for different dt------------
# ax,fig = u.create_figure_and_apply_format(figsize=(10, 8),ylabel=r"$\theta(t)$ [rad]", xlabel=r'$t$ [s]', grid_bool=False)

# for i in [1,10,-1] :     
#     ax.plot(thetasimul_list[i],thetadotsimul_list[i],linestyle = "-", label = r"$N_{steps}$" + f" = {nsteps[i]}")

# plt.tight_layout()
# ax.grid()
# u.set_legend_properties(ax)
# u.savefig(fig, "ExcitationChaos_phase_diffNsteps",ext)
